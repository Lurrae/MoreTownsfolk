namespace MoreTownsfolk.NPCs
{
	public struct BannerQuest
	{
		...
		/// <summary>
		/// Any banners which should give much faster progress towards a quest.
		/// <br/> For example, this is used to make four Pinky Banners (or 2 Pinky Banners and 50 other Slime banners) fulfill the Slime Staff quest.
		/// </summary>
		public Dictionary<int, int> ValuableBanners;
		/// <summary>
		/// The number of banners the player must provide to fulfill this quest.
		/// <br/> Any banner types in <see cref="AcceptedBannerTypes"/> can be used, including mixtures of different banners.
		/// </summary>
		public int Cost;
		/// <summary>
		/// The amount of time, in in-game days, that the Ninja must spend before returning to give the player their reward.
		/// <br/> Can be set to a value like <c>0.5f</c> to have the Ninja return later the same day instead of having to wait a full day.
		/// </summary>
		...
		public BannerQuest(int[] inputItems, int reward, int inputCost = 1, float time = 1.0f, int quantity = 1, bool infinite = false, Dictionary<int, int> valueBanners = null)
		{
			...
			if (valueBanners != null)
				ValuableBanners = valueBanners;
		}
	...
	public class NinjaBannerQuests
	{
		...
		public static readonly List<BannerQuest> Quests = new()
		{
			...
			new BannerQuest(SlimeBanners, ItemID.SlimeStaff, inputCost: 100, valueBanners: new Dictionary<int, int>() { {ItemID.PinkyBanner, 25 } }),
			new BannerQuest([ItemID.ZombieBanner, ItemID.ZombieEskimoBanner, ItemID.RaincoatZombieBanner], ItemID.ZombieArm, inputCost: 5),
			new BannerQuest([ItemID.HarpyBanner], ItemID.GiantHarpyFeather, inputCost: 3),

			// Earlygame underground quests
			new BannerQuest([ItemID.BatBanner], ItemID.ChainKnife, inputCost: 5),
			new BannerQuest([ItemID.BatBanner, ItemID.IceBatBanner, ItemID.JungleBatBanner, ItemID.SporeBatBanner, ItemID.GiantBatBanner], ItemID.DepthMeter, inputCost: 2),
			...
			new BannerQuest([ItemID.WormBanner], ItemID.WhoopieCushion),

			// Pre-HM Blood Moon quests
			new BannerQuest([ItemID.BloodZombieBanner, ItemID.DripplerBanner], ItemID.SharkToothNecklace, inputCost: 3),

			// Pre-HM Underworld quests
			new BannerQuest([ItemID.HellbatBanner, ItemID.LavaBatBanner], ItemID.MagmaStone, inputCost: 3, valueBanners: new Dictionary<int, int>() { {ItemID.LavaBatBanner, 3 } }),
			...
			new BannerQuest([ItemID.IceTortoiseBanner], ItemID.FrozenTurtleShell),
			...
			new BannerQuest([ItemID.AngryTrapperBanner], ItemID.Uzi, inputCost: 2),

			// Post-Mech Underworld quests
			new BannerQuest([ItemID.RedDevilBanner], ItemID.FireFeather),

			// Post-Plantera Dungeon quests
			new BannerQuest([ItemID.BlueArmoredBonesBanner, ItemID.HellArmoredBonesBanner, ItemID.RustyArmoredBonesBanner], ItemID.MaceWhip, inputCost: 4),
			new BannerQuest([ItemID.BlueArmoredBonesBanner, ItemID.HellArmoredBonesBanner, ItemID.RustyArmoredBonesBanner], ItemID.WispinaBottle, inputCost: 4),
			new BannerQuest([ItemID.BlueArmoredBonesBanner, ItemID.HellArmoredBonesBanner, ItemID.RustyArmoredBonesBanner], ItemID.Keybrand, inputCost: 4),
			new BannerQuest([ItemID.BlueArmoredBonesBanner, ItemID.HellArmoredBonesBanner, ItemID.RustyArmoredBonesBanner], ItemID.MagnetSphere, inputCost: 6),
			new BannerQuest([ItemID.BlueArmoredBonesBanner, ItemID.HellArmoredBonesBanner, ItemID.RustyArmoredBonesBanner], ItemID.BoneFeather, inputCost: 6)
			#endregion
		};
	}
}